...
const AffichierProduit = () => {
  ...
  const handleSuppremier = async (e, p) => {
    ...
    const jso = await response.json();
    if (!response.ok) {
      console.log('erreur de suppression des produits');
    } else {
      console.log('suppression établie');
  ...
  return (
    <>
      <NavBarAdmin />
      <Search onSearchResults={(results) => setProducts(results)} />
      <div className="Box">
        <div className="Side">
          <SidebarProduct />
        </div>
              ...
              <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
                {products.map((product) => (
                  <div key={product._id}>
                    ...
                    <button onClick={(e) => handleUpdate(e, product)} className="btn btn-primary ">
                      <MdModeEditOutline />
                    </button>
                    <button type="button" onClick={(e) => handleSuppremier(e, product)} className="btn btn-danger">
                      <AiFillDelete />
                    </button>
                  </div>
                ))}
